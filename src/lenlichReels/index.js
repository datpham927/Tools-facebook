...
const VIDEO = require("./data/tamconcophat");
const path = require("path");
const { formatShopeeText } = require("../utils/url");
...
async function scheduleVideos(driver, VIDEO) {
  ...
  const SLOTS_PER_DAY =
    Math.floor((END_HOUR - START_HOUR) / HOURS_INTERVAL) + 1; // Number of 2-hour slots in a day
  ...
  for (let i = 0; i < VIDEO.length; i++) {
    try {
      ...
      // Schedule post
      const title = await formatShopeeText(path.parse(videoPath).name);
      // const text = await generateChatContent(title);
      await createPost(
        driver,
        formattedDate,
        `${targetHour}`,
